...
async function generateAiAgentDecision({ waId, profileName, latestMessageText, aiState }) {
    ...
    const prompt = [
        ...
        '- Treat the latest client message as the highest-priority source of truth.',
        '- If the client mentions a new subject, new assignment, or different task, immediately drop any previous subject assumption and switch to the new task context.',
        '- Do not keep pushing an older topic from chat history when the client has clearly introduced a new one.',
        '- Example: if earlier discussion was data science but the client now says maths assignment, respond for maths only.',
        '- When the client says they have an assignment/project/task in a subject, ask for the specific topic, deadline, and instruction file if those details are not already available.',
        '- If the client already shared the subject, do not ask which subject again; ask the next useful detail.',
...
app.post('/api/razorpay/payment-link/refresh', async (req, res) => {
    try {
        ...
        await refreshRazorpayPaymentStatuses(orderId, { force: req.body?.force === true });
        return res.json({
            ok: true,
            razorpayPaymentsByOrder: crmState.razorpayPaymentsByOrder,
...
async function refreshRazorpayPaymentLinkStatus(orderId, link, options = {}) {
    if (!isRazorpayConfigured() || !shouldRefreshRazorpayLink(link, options.force === true)) return false;
...
async function refreshRazorpayPaymentStatuses(targetOrderId = '', options = {}) {
    ...
    for (const [orderId, row] of entries) {
        ...
        for (const link of links.slice(0, 5)) {
            changed = (await refreshRazorpayPaymentLinkStatus(orderId, link, options)) || changed;
        }
    }
    return changed;
}
